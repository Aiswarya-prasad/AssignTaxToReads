...
column_ba = HeatmapAnnotation(SDP = SDP_names,
                              Genus = genus_names,
                              Strain = strain_name,
                       col = list(SDP = make_col_list(SDP_names, "Spectral"),
                                  Genus = make_col_list(genus_names, "Set1"),
                                  Strain = make_col_list(strain_name, "Pastel1")
                       ), border = T
                    )
...
heatmap_obj = Heatmap(abundance_mat_alq,
        col = colorRamp2(c(0, 1000, 10000, 100000, 1000000, 1000000, 10000000), 
...
strain_name <- colnames(abundance_mat_samples) %>% as.data.frame() %>% left_join(psmelt(ps) %>% ungroup() %>% select(OTU, SDP, Strain) %>% unique %>% mutate(Strain_name = paste0(SDP, "--", Strain)), by = c(. = "OTU")) %>% pull(Strain_name)
column_ba = HeatmapAnnotation(SDP = SDP_names,
                              Genus = genus_names,
                       col = list(SDP = make_col_list(SDP_names, "Spectral"),
                                  Genus = make_col_list(genus_names, "Set1")
