...
for (i in 1:n_length) {
  g <- ggplot() +
  geom_bar(data = df_plot_by_strain_alq %>% filter(SDP == sdp_list[[i]]),
           aes(x = factor(Sample, ID_order),
               y = abs_ab,
               fill = Strain
          ), stat = "identity", position = "stack", color = "black", size = 0.1
...
for (i in 1:(n_length)) {
  g <- ggplot() +
  geom_bar(data = df_plot_by_strain_alq %>% filter(SDP_Species == species_list[[i]]),
           aes(x = factor(Sample, ID_order),
               y = abs_ab,
               fill = Strain_header,
          ), stat = "identity", position = "stack", color = "black", size = 0.1
...
Treatment_factor_order <- c("AB-0", "AB-1", "AB-3", "A-3", "BA-1", "BA-3", "B-3")
...
for (i in 1:n_length) {
  g <- ggplot() +
  geom_bar(data = sub_df_plot_by_strain %>% filter(SDP == sdp_list[[i]]) %>% filter(Treatment %in% Treatment_factor_order),
           aes(x = factor(Sample, ID_order),
               y = rel_ab,
               fill = Strain
          ), stat = "identity", position = "stack", color = "black", size = 0.1
...
# Plot by strain across samples and treatments
sdp_list <- unique(df_plot_by_strain$SDP)
sdp_list <- sdp_list[which(!is.na(sdp_list))]
n_length <- length(sdp_list[which(!is.na(sdp_list))])
g_list <- c()
for (i in 1:n_length) {
  g <- ggplot() +
  geom_bar(data = df_plot_by_strain %>% filter(SDP == sdp_list[[i]]),
           aes(x = factor(Sample, ID_order),
               y = abs_ab,
               fill = Strain
          ), stat = "identity", position = "stack", color = "black", size = 0.1
...
grid.arrange(grobs = g_list, ncol = 2, nrow = 6)
pdf("04_Results/Figures/Abundance_samples_per_strain_by_treatment_db_strains.pdf", width = 10, height = 20)
grid.arrange(grobs = g_list, ncol = 2, nrow = 6)
dev.off()
```
