...
source("/nas/FAC/FBM/DMF/pengel/spirit/D2c/aprasad/20220921_aprasad_PriorityEffectsExperimentPilot/03_PilotExperiment/05_qPCRs/qPCR_parse.R") # set prefix directory as needed within the scripts
...
## Number of reads obtained from sequencing

```{r 1}
# get number of reads from summary files
df_assigned_reads_raw <- data.frame()
for (file in list.files("04_assign_reads_to_strain")) {
  if (grepl("summary", file)) {
    sample = strsplit(file, "_summary")[[1]][[1]]
    lines = readLines(paste0("04_assign_reads_to_strain/", file), n = 10)
    for (line in lines) {
      if (grepl("total_reads", line)) {
        total_reads = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("unknown", line)) {
        unknown = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("assigned_unambiguous", line)) {
        assigned_unambiguous = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("assigned_ambiguous", line)) {
        assigned_ambiguous = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("within", line)) {
        within_range = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("shorter", line)) {
        shorter = strsplit(line, "=")[[1]][[2]]
      }
      if (grepl("longer", line)) {
        longer = strsplit(line, "=")[[1]][[2]]
      }
    }
    vector_to_add <- c(sample, total_reads, unknown, assigned_unambiguous, assigned_ambiguous, within_range, shorter, longer)
    df_assigned_reads_raw <- rbind(df_assigned_reads_raw, vector_to_add)
  }
}
colnames(df_assigned_reads_raw) <- c("sample", "total_reads", "unknown", "assigned_unambiguous", "assigned_ambiguous", "within_range", "shorter", "longer")
df_assigned_reads <- df_assigned_reads_raw %>%
                      mutate_at(vars(!contains("sample")), as.numeric) %>%
                      left_join(df_meta, by = c("sample" = "ID"))

ggplot() +
  geom_bar(data = df_assigned_reads,
           aes(y = factor(sample, ID_order),
               x = total_reads,
  ...
  labs(x = "Number of raw reads", y = "Sample", fill = "Treatment") +
    ...
    ggsave("Figures/Number_of_reads_by_treatment.pdf")
```


```{r 2}
df_assigned_reads_plot <- df_assigned_reads %>%
                            select(sample, Treatment, unknown, assigned_unambiguous, assigned_ambiguous) %>%
                            pivot_longer(!c(sample, Treatment), names_to = "Type", values_to = "Count")
df_assigned_reads_lengths <- df_assigned_reads %>%
                            select(sample, Treatment, shorter, within_range, longer) %>%
                            pivot_longer(!c(sample, Treatment), names_to = "Type", values_to = "Count")

ggplot() +
  geom_point(data = df_assigned_reads_plot,
           aes(x = -500,
               y = factor(sample, ID_order),
               color = Treatment,
               size = 3
           ), 
           position = "stack", stat = "identity"
  ) +
  geom_bar(data = df_assigned_reads_plot,
           aes(y = sample,
               x = Count,
               fill = Type
           ), stat = "identity"
  ...
  labs(x = "Number of raw reads", y = "Sample", fill = "Type") +
    scale_color_manual(values=treatment_order_color) +
    scale_x_continuous(labels=unit_format(unit = "K", scale = 1e-3)) +
    make_theme(setFill = T, setCol = F, guide_nrow = 3)
    ggsave("Figures/Number_of_reads_by_type.pdf")

ggplot() +
  geom_point(data = df_assigned_reads_lengths,
           aes(x = -500,
               y = factor(sample, ID_order),
               color = Treatment,
               size = 3
  ...
  geom_bar(data = df_assigned_reads_lengths,
           aes(y = sample,
               x = Count,
               fill = Type
           ), stat = "identity"
  ...
  labs(x = "Number of raw reads", y = "Sample", fill = "Type") +
    ...
    scale_x_continuous(labels=unit_format(unit = "K", scale = 1e-3)) +
    make_theme(setFill = T, setCol = F, guide_nrow = 3)
    ggsave("Figures/Number_of_reads_by_type_lengths.pdf")
...
ggplot() +
  ...
  labs(x = "Number of raw reads", y = "Sample", fill = "Type") +
    ...
    scale_x_continuous(labels=unit_format(unit = "K", scale = 1e-3)) +
    make_theme(setFill = T, setCol = F, guide_nrow = 3)
    ggsave("Figures/Number_of_reads_by_type_percent.pdf")
```
...
This will be used here to parse the output of `assign_reads_to_strains.py` which can be found in `04_assign_reads_to_strain/strain_counts_raw.csv`.
...
#                                            --output_path \"04_assign_reads_to_strain\"")
...
get_sdp_from_strain <- function(x) {
  #  x is in the format x="ESL0394.1_firm4_1"
  if (x == "unknown" || x == "Unknown") {
    return(NA)
  }  
  SDP <- df_strain_counts_raw %>%
          colnames %>%
          as.data.frame() %>%
          filter(grepl(x, .)) %>%
          head(1) %>%
...
df_strain_counts_raw <- data.frame()
for (file in list.files("04_assign_reads_to_strain")) {
  if (grepl("_strain_counts.csv", file)) {
    sample = strsplit(file, "_strain_counts")[[1]][[1]]
    df_strain_counts_raw_read <- read.table(paste0("04_assign_reads_to_strain/", file), row.names = NULL, sep = ",") %>%
                          select(V2, V3)
    df_strain_counts_raw <- rbind(df_strain_counts_raw, c(sample, df_strain_counts_raw_read[-1,]$V3))
    colnames(df_strain_counts_raw) <- c("Sample", df_strain_counts_raw_read[-1,]$V2)
  }
}
...
# names(df_strain_counts_raw)
df_strain_counts_raw_all <- df_strain_counts_raw %>%
                            mutate_at(vars(contains("ESL")), as.numeric) %>%
                            mutate(`unknown` = as.numeric(unknown)) %>%
                              pivot_longer(!Sample, values_to = "Count", names_to = "Strain") %>%
                              mutate(Community = ifelse(Vectorize(get_strain_from_id)(Strain) %in% community_a, "a", NA)) %>%
                              mutate(Community = ifelse(Vectorize(get_strain_from_id)(Strain) %in% community_b, "b", Community)) %>%
                              mutate(Community = ifelse(Vectorize(get_strain_from_id)(Strain) %in% community_ab, "ab", Community)) %>%
                              left_join(df_meta_pilot, by = c("Sample" = "ID"))
df_strain_counts <- df_strain_counts_raw %>%
                      mutate_at(vars(contains("ESL")), as.numeric) %>%
                      mutate(`ESL0825` = `ESL0825-1_bifido_1.1`) %>%
                      mutate(`ESL0822` = `ESL0822-1_bifido_1.3`) %>%
                      mutate(`ESL0824` = `ESL0824-1_bifido_1.4.1`/2) %>%
                      mutate(`ESL0827` = `ESL0827-1_bifido_1.4.2`) %>%
                      mutate(`ESL0200` = `ESL0200-1_bifido_1.5`/2) %>%
                      mutate(`ESL0197` = `ESL0197-1_bifido_2`/2) %>%
                      mutate(`ESL0295` = `ESL0295-1_firm4_1`) %>%
                      mutate(`ESL0185` = `ESL0185-1_firm5_1`/4) %>%
                      mutate(`ESL0183` = mean(c(`ESL0183-1_firm5_2`, `ESL0183-2_firm5_2`, `ESL0183-3_firm5_2`))) %>%
                      mutate(`ESL0184` = mean(c(`ESL0184-1_firm5_3`, `ESL0184-2_firm5_3`, `ESL0184-3_firm5_3`, `ESL0184-4_firm5_3`))) %>%
                      mutate(`ESL0186` = mean(c(`ESL0186-1_firm5_4`, `ESL0186-2_firm5_4`))/2) %>%
                      mutate(`ESL0820` = `ESL0820-1_bifido_1.1`/2) %>%
                      mutate(`ESL0170` = `ESL0170-1_bifido_1.3`/2) %>%
                      mutate(`ESL0198` = `ESL0198-1_bifido_1.4.1`) %>%
                      mutate(`ESL0199` = `ESL0198-1_bifido_1.4.1`/2) %>%
                      mutate(`ESL0819` = `ESL0819-2_bifido_1.5`/2) %>%
                      mutate(`ESL0394` = `ESL0394-1_firm4_1`) %>%
                      mutate(`ESL0263` = `ESL0263-1_firm5_1`) %>%
                      mutate(`ESL0262` = ifelse(`ESL0262-1_firm5_2` > `ESL0183`, `ESL0262-1_firm5_2`-`ESL0183`, `ESL0262-1_firm5_2`)) %>%
                      mutate(`ESL0350` = `ESL0350-1_firm5_3`) %>%
                      mutate(`ESL0261` = `ESL0261-1_firm5_4`) %>%
                      mutate(`Unknown` = as.numeric(unknown)) %>%
                      select(Sample, Unknown, all_of(genome_names)) %>%
                      pivot_longer(!Sample, values_to = "Count", names_to = "Strain") %>%
                      mutate(SDP = Vectorize(get_sdp_from_strain)(Strain)) %>%
                      group_by(Sample, Strain) %>% 
                      summarise(Sample, Strain, SDP, Count = sum(Count)) %>%
                              mutate(Community = ifelse(Strain %in% community_a, "a", NA)) %>%
                              mutate(Community = ifelse(Strain %in% community_b, "b", Community)) %>%
                              mutate(Community = ifelse(Strain %in% community_ab, "ab", Community)) %>%
                              left_join(df_meta_pilot, by = c("Sample" = "ID"))
...
# df_strain_counts_raw %>% filter(Sample == "1-16")
# df_assigned_reads_raw %>% filter(sample == "1-16")
# df_strain_counts %>% filter(Sample == "1-16") %>% pull(Count)
# df_strain_counts %>% filter(Sample == "1-16") %>% pull(Community)
# df_strain_counts_raw_all %>% filter(Sample == "3-5") %>% select(Community, Strain, Count) %>% print(n = 60)
# df_strain_counts %>% filter(Sample == "3-5") %>% select(Community, Strain, Count) %>% print(n = 60)
...
ggplot() +
  ...
  # geom_vline(xintercept = 3000) +
  labs(x = "Count", y = "Sample", fill = "SDP", color = "Treatment") +
  scale_color_manual(values=treatment_order_color) +
  make_theme(setCol = F, setFill = T, guide_nrow = 6, palettefill = "Spectral")
  ggsave("Figures/Plot_sdp_per_sample.pdf")
...
ggplot() +
  ...
  geom_bar(data = df_strain_counts %>% 
           ...
           aes(x = Count,
               y = factor(Sample, ID_order),
               color = factor(Community, c("a", "b", "ab")),
               fill = SDP
           ),
           position = "stack", stat = "identity"
  ) +
  labs(x = "Count", y = "Sample", fill = "SDP", color = "Treatment") +
  scale_color_manual(values = c("#377eb8", "#e41a1c", "#984ea3"), labels = c("a", "b", "ab")) +
  make_theme(setCol = F, setFill = T, guide_nrow = 6, palettefill = "Spectral")
  ggsave("Figures/Plot_sdp_per_sample_assigned_community.pdf")
...
ggplot() +
  ...
  labs(x = "Count", y = "Sample", fill = "SDP", color = "Treatment") +
  make_theme(setCol = F, setFill = T, guide_nrow = 6, palettefill = "Spectral")
  ggsave("Figures/Plot_sdp_per_sample_assigned.pdf")
...
ggplot() +
  ...
  geom_bar(data = df_strain_counts,
           aes(x = Count,
               y = factor(Sample, ID_order),
               fill = factor(Community, c("a", "b", "ab"))
           ), 
           position = "stack", stat = "identity"
  ) +
  # geom_vline(xintercept = 3000) +
  labs(x = "Count", y = "Sample", fill = "Community", color = "Treatment") +
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  scale_color_manual(values=treatment_order_color) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_sample.pdf")
...
ggplot() +
  ...
  scale_color_manual(values=treatment_order_color) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_sample_assigned.pdf")
...
ggplot() +
  ...
  labs(x = "Percentage", y = "Sample", fill = "Community") +
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  scale_color_manual(values=treatment_order_color) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_sample_percentage.pdf")
...
ggplot() +
  ...
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  scale_color_manual(values=treatment_order_color) +
  labs(x = "Count", y = "Treatment", fill = "Community") +
  make_theme(setCol = F, setFill = F, guide_nrow = 4)
  ggsave("Figures/Plot_community_per_sample_assigned_percentage.pdf")
...
ggplot() +
  ...
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_treatment.pdf")
...
ggplot() +
  ...
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_treatment_assigned.pdf")
...
ggplot() +
  ...
  scale_fill_manual(values = c("#b3cde3", "#fbb4ae", "#decbe4"), labels = c("a", "b", "ab")) +
  make_theme(setCol = F, setFill = F)
  ggsave("Figures/Plot_community_per_treatment_percentage.pdf")
...
ggplot() +
  ...
  ggsave("Figures/Plot_community_per_treatment_assigned_percentage.pdf")
```
