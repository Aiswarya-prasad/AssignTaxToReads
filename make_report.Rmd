...
for (file in list.files("03_assign_reads_to_strain")) {
  if (grepl("summary", file)) {
    ...
    lines = readLines(paste0("03_assign_reads_to_strain/", file), n = 15)
    total_reads = strsplit(lines[3], "=")[[1]][[2]]
    unknown = strsplit(lines[7], "=")[[1]][[2]]
    assigned_unambiguous = strsplit(lines[8], "=")[[1]][[2]]
    assigned_ambiguous = strsplit(lines[9], "=")[[1]][[2]]
    vector_to_add <- c(sample, total_reads, unknown, assigned_unambiguous, assigned_ambiguous)
    df_assigned_reads_raw <- rbind(df_assigned_reads_raw, vector_to_add)
  }
}
colnames(df_assigned_reads_raw) <- c("sample", "total_reads", "unknown", "assigned_unambiguous", "assigned_ambiguous")
...
df_strain_counts %>% filter(Sample == "3-8") %>% pull(Count)
df_strain_counts %>% filter(Sample == "3-8") %>% pull(Community)
...
df_strain_counts %>%
                    ...
                    group_by(Treatment) %>%
                    mutate(Percentage = Count/sum(Count)*100) %>%
                    filter(Treatment == "alq-B") %>%
                    select(Strain, Community, Count) %>%
                    print(n = 40)
```
