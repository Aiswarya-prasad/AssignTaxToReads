...
for (file in list.files("03_assign_reads_to_strain")) {
  if (grepl("summary", file)) {
    sample = strsplit(file, "_summary")[[1]][[1]]
    lines = readLines(paste0("03_assign_reads_to_strain/", file), n = 10)
    for (line in lines) {
...
for (file in list.files("03_assign_reads_to_strain")) {
  if (grepl("_strain_counts.csv", file)) {
    ...
    df_strain_counts_raw_read <- read.table(paste0("03_assign_reads_to_strain/", file), row.names = NULL, sep = ",") %>%
                          select(V2, V3)
